...
object DynamicArrays {
  def main(args: Array[String]): Unit = {
    ...
    /* As we already have the sum no need to calculate again.
     If you did not have this you would use the above method
     and then divide the sum by the length. */
    println(f"The average is : ${sum.toDouble / nums.length}%.2f")
